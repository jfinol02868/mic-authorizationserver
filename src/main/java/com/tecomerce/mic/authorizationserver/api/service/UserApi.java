...
public interface UserApi {
    ...
    @ApiResponse(responseCode = "403", content = @Content(mediaType = MEDIA_TYPE,  schema = @Schema(implementation = MessageResponseDTO.class)))
    @ApiResponse(responseCode = "500", content = @Content(mediaType = MEDIA_TYPE,  schema = @Schema(implementation = MessageResponseDTO.class)))
    default ResponseEntity<List<UserDTO>> findAllPaginated(@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size,@RequestParam(name = "username", defaultValue = "username") String sort, SortEnumDTO direction) {
    ...
    default ResponseEntity<List<UserDTO>> filters(
            @RequestParam(required = false, defaultValue = "{\"username\":\"user\", \"expired\":\"false\"}") String filterProperties,
